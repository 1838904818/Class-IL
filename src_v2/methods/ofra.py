...
DEFAULT_PREDICTION_ARMS: dict[str, dict[str, float]] = {
    "p-only": {"head_weight": 1.0, "router_weight": 0.0},
    "z-only": {"head_weight": 0.0, "router_weight": 1.0},
    "joint": {"head_weight": 1.0, "router_weight": 0.5},
}
PREDICTION_CALIBRATION = "softmax_prob"


def normalize_prediction_arms(
    prediction_arms: Mapping[str, Mapping[str, float]] | None,
) -> dict[str, dict[str, float]]:
    """Validate and copy named head/router weighting configurations."""
    source = DEFAULT_PREDICTION_ARMS if prediction_arms is None else prediction_arms
    if not isinstance(source, Mapping) or not source:
        raise ValueError("prediction_arms must be a non-empty mapping")
    ...
    for raw_name, raw_config in source.items():
        ...
        if name in normalized:
            raise ValueError(f"duplicate prediction arm name: {name!r}")
        if not isinstance(raw_config, Mapping):
            raise ValueError(
                f"prediction arm {name!r} must map weights by name"
            )
        missing = {
            key for key in ("head_weight", "router_weight")
            if key not in raw_config
        }
        if missing:
            raise ValueError(
                f"prediction arm {name!r} is missing weights: {sorted(missing)}"
            )
        head_weight = float(raw_config["head_weight"])
        router_weight = float(raw_config["router_weight"])
        for weight_name, value in (
            ...
            if not np.isfinite(value) or value < 0:
                raise ValueError(
                    f"prediction arm {name!r} {weight_name} must be a "
                    "non-negative finite value"
                )
        if head_weight == 0 and router_weight == 0:
            raise ValueError(
                f"prediction arm {name!r} cannot set both weights to zero"
            )
        normalized[name] = {
            "head_weight": head_weight,
            "router_weight": router_weight,
...
def compute_classification_diagnostics(
    ...
    """Return JSON-safe multiclass metrics for an explicit label order."""
    ...
    if len(y_true) == 0:
        raise ValueError("classification diagnostics require at least one row")
    ...
    unknown_true = sorted(set(map(int, y_true)) - set(ordered_labels))
    unknown_pred = sorted(set(map(int, y_pred)) - set(ordered_labels))
    if unknown_true:
        raise ValueError(f"y_true contains labels outside labels: {unknown_true}")
    if unknown_pred:
        raise ValueError(f"y_pred contains labels outside labels: {unknown_pred}")

    true_indices = np.fromiter(
        (label_to_index[int(value)] for value in y_true),
        dtype=np.int64,
        count=len(y_true),
    )
    pred_indices = np.fromiter(
        (label_to_index[int(value)] for value in y_pred),
        dtype=np.int64,
        count=len(y_pred),
    )
    matrix = np.zeros(
    ...
    recall = np.divide(
        true_positive,
        support,
        out=np.zeros(len(ordered_labels), dtype=np.float64),
        where=support > 0,
    )
    precision = np.divide(
        true_positive,
    ...
    return {
        ...
        "confusion_matrix": {
            ...
            "class_names": [resolved_names[label] for label in ordered_labels],
            "values": matrix.tolist(),
        },
        "per_class": [
            {
                "label": label,
                "class_name": resolved_names[label],
...
class RawExemplarBuffer:
    ...
    def add(self, family: str, X: np.ndarray, y: np.ndarray, embeddings: np.ndarray):
        ...
        if k == 0:
            return
        # Greedy farthest-first selection with bounded temporary memory.
        sel = [int(np.random.randint(0, len(embeddings)))]
        embeds_arr = np.asarray(embeddings, dtype=np.float32)
        selected = np.zeros(len(embeds_arr), dtype=bool)
        min_dist_sq = np.full(len(embeds_arr), np.inf, dtype=np.float32)
        distance_batch_size = 65_536
        while len(sel) < k:
            newest = sel[-1]
            for start in range(0, len(embeds_arr), distance_batch_size):
                stop = min(start + distance_batch_size, len(embeds_arr))
                delta = embeds_arr[start:stop] - embeds_arr[newest]
                dist_sq = np.einsum("ij,ij->i", delta, delta)
                np.minimum(
                    min_dist_sq[start:stop],
                    dist_sq,
                    out=min_dist_sq[start:stop],
                )
            selected[newest] = True
            min_dist_sq[selected] = -np.inf
            sel.append(int(np.argmax(min_dist_sq)))
...
class OFRAAgent(nn.Module):
    ...
    def __init__(
        ...
        router_fit_max_samples: int | None = None,
        ft_heads: int = 4,
        ft_dim_head: int = 16,
        ...
        self.router = DPMeansRouter(
            ...
            max_fit_samples=router_fit_max_samples,
        )
        ...
        self.checkpoint_diagnostics: dict | None = None
    ...
    def fit_input_stats(self, X: np.ndarray):
        ...
        # Accumulate in float64. Some NetFlow rate fields contain very large
        # but finite float32 values; float32 sums and squared deviations can
        # overflow even though every input value passes the finite-value check.
        mean = X.mean(axis=0, dtype=np.float64).astype(np.float32)
        std = X.std(axis=0, dtype=np.float64).astype(np.float32)
        if not np.isfinite(mean).all() or not np.isfinite(std).all():
            raise ValueError(
                "feature statistics are not representable as finite float32 values"
            )
        std[std < 1e-6] = 1.0
    ...
    def supervised_pretrain_encoder(
        ...
        instead).
        """
        if not isinstance(self.encoder, (MLPEncoder, FTTransformerEncoder)):
            if verbose:
                print("  supervised_pretrain skipped (unsupported encoder)")
            return
        X = self._validate_features(X, name="X")
        y = np.asarray(y)
    ...
    def embed(self, X: np.ndarray, batch: int = BATCH_SIZE,
        ...
        if not normalized:
            X = self._normalize(X)
        dev = next(self.encoder.parameters()).device
        outputs = np.empty((len(X), self.d_model), dtype=np.float32)
        for i in range(0, len(X), batch):
            stop = min(i + batch, len(X))
            bx = torch.from_numpy(
                np.ascontiguousarray(X[i:stop], dtype=np.float32)
            ).to(dev, non_blocking=(dev.type == "cuda"))
            outputs[i:stop] = self.encoder(bx).cpu().numpy()
        return outputs
    ...
    def predict(
        ...
        self.encoder.eval()
        embeds_np = self.embed(X, batch=batch_size)
        N = len(X)
        ...
        for start in range(0, N, batch_size):
            ...
            embeds = torch.from_numpy(embeds_np[start:stop]).to(
                dev,
                non_blocking=(dev.type == "cuda"),
            )
            for fi, fname in enumerate(self.pool.families):
                logits = self.pool.heads[fname](embeds)
                if calibration == "softmax_prob":
                    values = torch.softmax(logits, dim=1)[:, 1]
                else:
                    values = logits[:, 1] - logits[:, 0]
                head_scores[start:stop, fi] = values.cpu().numpy()
        ...
        for fi, fname in enumerate(self.pool.families):
            router_scores[:, fi] = -self.router.distances_to_family(
                embeds_np,
                fname,
            )
    ...
    def predict_arms(
        self,
        X: np.ndarray,
        prediction_arms: Mapping[str, Mapping[str, float]] | None = None,
        *,
        calibration: str = PREDICTION_CALIBRATION,
        batch_size: int = 4096,
    ) -> dict[str, np.ndarray]:
        """Evaluate named weighting arms from one shared scoring pass."""
        arms = normalize_prediction_arms(prediction_arms)
        if calibration not in {"softmax_prob", "raw_scalar"}:
            raise ValueError(
                f"Unsupported calibration={calibration!r}; expected "
                "'softmax_prob' or 'raw_scalar'"
        ...
        if len(X) == 0 or not self.pool.families:
            return {
                name: np.full(len(X), -1, dtype=np.int64)
                for name in arms
            }
        ...
        bad_centroids = []
        for family in self.pool.families:
            centroids = self.router.centroids.get(family)
            if (
                not isinstance(centroids, np.ndarray)
                or centroids.ndim != 2
                or len(centroids) == 0
                or centroids.shape[1] != self.d_model
        ...
        self.encoder.eval()
        embeddings = self.embed(X, batch=batch_size)
        n_rows = len(X)
        n_families = len(self.pool.families)

        head_scores = np.zeros((n_rows, n_families), dtype=np.float32)
        device = next(self.encoder.parameters()).device
        for start in range(0, n_rows, batch_size):
            stop = min(start + batch_size, n_rows)
            batch_embeddings = torch.from_numpy(embeddings[start:stop]).to(
                device,
                non_blocking=(device.type == "cuda"),
            )
            for family_index, family in enumerate(self.pool.families):
                logits = self.pool.heads[family](batch_embeddings)
                if calibration == "softmax_prob":
                    values = torch.softmax(logits, dim=1)[:, 1]
                else:
                    values = logits[:, 1] - logits[:, 0]
                head_scores[start:stop, family_index] = values.cpu().numpy()
        if not np.isfinite(head_scores).all():
            raise FloatingPointError("Non-finite family-head scores encountered")

        router_scores = np.empty((n_rows, n_families), dtype=np.float32)
        for family_index, family in enumerate(self.pool.families):
            router_scores[:, family_index] = -self.router.distances_to_family(
                embeddings,
                family,
            )
        if not np.isfinite(router_scores).all():
            raise FloatingPointError("Non-finite router scores encountered")
        router_mean = router_scores.mean(axis=1, keepdims=True)
        router_std = router_scores.std(axis=1, keepdims=True) + 1e-8
        router_scores_z = (router_scores - router_mean) / router_std
        if not np.isfinite(router_scores_z).all():
            raise FloatingPointError(
                "Non-finite standardized router scores encountered"
            )

        family_classes = np.asarray(
            [
                self.family_to_class[family]
                for family in self.pool.families
            ],
            dtype=np.int64,
        )
        ...
        for name, config in arms.items():
            scores = (
                config["head_weight"] * head_scores
                + config["router_weight"] * router_scores_z
            )
            if not np.isfinite(scores).all():
                raise FloatingPointError(
                    f"Non-finite prediction scores encountered for arm {name!r}"
                )
            predictions[name] = family_classes[scores.argmax(axis=1)]
        return predictions
...
def run_ofra(
    ...
    When ``prediction_arms`` is provided, ``agent.checkpoint_diagnostics``
    stores seen-class metrics after every task. The returned ``acc_matrix``
    remains the original joint-score task-accuracy matrix.
    """
    if encoder_type not in {"transformer", "mlp", "ft_transformer"}:
        raise ValueError(
            f"Unsupported encoder_type={encoder_type!r}; expected "
            "'transformer', 'mlp', or 'ft_transformer'"
        )
    if loss_fn not in {"focal", "ce"}:
        raise ValueError(
            f"Unsupported loss_fn={loss_fn!r}; expected 'focal' or 'ce'"
    ...
    diagnostic_arms = (
        normalize_prediction_arms(prediction_arms)
        if prediction_arms is not None
        else None
    )
    if diagnostic_arms is not None and "joint" in diagnostic_arms:
        if diagnostic_arms["joint"] != DEFAULT_PREDICTION_ARMS["joint"]:
            raise ValueError(
                "the diagnostic arm named 'joint' must use head_weight=1.0 "
                "and router_weight=0.5 to match acc_matrix"
            )
    if class_names is None:
        resolved_class_names = [str(index) for index in range(n_classes)]
    else:
        resolved_class_names = [str(value) for value in class_names]
        if len(resolved_class_names) != n_classes:
            raise ValueError(
                "class_names must contain exactly n_classes entries"
            )
    class_name_by_label = {
        index: name for index, name in enumerate(resolved_class_names)
    }
    if str(device) == "auto":
    ...
    for task_index, task in enumerate(tasks):
        ...
        if any(int(c) < 0 or int(c) >= n_classes for c in task):
            raise ValueError(
                f"tasks[{task_index}] contains a class outside "
                f"[0, {n_classes - 1}]"
            )
    ...
    agent = OFRAAgent(
        ...
        router_fit_max_samples=router_fit_max_samples,
        ft_heads=ft_heads,
    ...
    if verbose:
        ...
        print(f"  Router fit limit: {router_fit_max_samples or 'uncapped'}")
    ...
    if encoder_type == "transformer" and pretrain_epochs > 0:
        ...
        encoder_type in {"mlp", "ft_transformer"}
        and supervised_pretrain_task0
        and len(tasks) > 0
    ):
    ...
    diagnostic_checkpoints: list[dict] = []
    seen_labels: list[int] = []
    for i, task_i in enumerate(tasks):
        task_labels = [int(label) for label in task_i]
        seen_labels.extend(task_labels)
        ...
        if diagnostic_arms is not None:
            X_seen, y_seen = subset_by_classes(X_te, y_te, seen_labels)
            if len(X_seen) == 0:
                raise ValueError(
                    f"checkpoint {i} has no test rows for seen labels "
                    f"{seen_labels}"
                )
            arm_predictions = agent.predict_arms(
                X_seen,
                diagnostic_arms,
                calibration=PREDICTION_CALIBRATION,
            )
            arm_results = {}
            for arm_name, predictions in arm_predictions.items():
                metrics = compute_classification_diagnostics(
                    y_seen,
                    predictions,
                    labels=seen_labels,
                    class_names=class_name_by_label,
                )
                arm_results[arm_name] = {
                    "head_weight": diagnostic_arms[arm_name]["head_weight"],
                    "router_weight": diagnostic_arms[arm_name]["router_weight"],
                    "calibration": PREDICTION_CALIBRATION,
                    **metrics,
                }
            diagnostic_checkpoints.append({
                "checkpoint": int(i),
                "checkpoint_name": f"after_task_{i}",
                "task": {
                    "index": int(i),
                    "labels": task_labels,
                    "class_names": [
                        class_name_by_label[label] for label in task_labels
                    ],
                },
                "seen_labels": list(seen_labels),
                "seen_class_names": [
                    class_name_by_label[label] for label in seen_labels
                ],
                "n_test_samples": int(len(y_seen)),
                "arms": arm_results,
            })

    if diagnostic_arms is not None:
        agent.checkpoint_diagnostics = {
            "schema_version": 1,
            "scope": "test samples from all classes seen by each checkpoint",
            ...
            "prediction_arms": {
                ...
                for name, config in diagnostic_arms.items()
            },
