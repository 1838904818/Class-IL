...
python -m sensitivity_overlap `
  --manifest C:\path\to\streaming_manifest.json `
  --output-dir C:\path\to\evaluation_views\duplicate_excluded `
  --view-name duplicate_excluded `
  --work-directory C:\path\to\disk_work
```
...
python -m streaming_full.exposure_preflight `
  ...
  --output C:\path\to\exposure_preflight.json
...
python -m streaming_full.summarize `
  --input-root C:\path\to\campaign-results `
  --output C:\path\to\campaign-results\paired_inference.json `
