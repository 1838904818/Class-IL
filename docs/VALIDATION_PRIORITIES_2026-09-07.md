...
- [Historical seed-1 sensitivity](../results/malaya-network-gt/etg-seed1/analysis.json): 64 drift settings already exist. This is not a five-seed, three-method recomputation of the sequential governance ledger.
...
| 2 | Attribution and policy sensitivity | Hash-verified arrays; fixed k, threshold and recall-guard grid; full ETG state replay and denominators for all seeds and methods | Historical single-seed drift grid only |
...
| Extension | Operational ETG | Offline analyst workflow and measurable benefit versus no ledger, with applicable ethics approval | Benefit unvalidated |
...
3. The historical seed-1 drift analyzer uses recall change strictly greater than -0.05 at the primary setting, not a symmetric five-pp stability band. Preserve original operators and tie handling in any derivative analysis.
...
6. Exact two-sided paired tests with five nonzero pairs have minimum p=0.0625. Report effects, uncertainty and all endpoints without claiming p<0.05 superiority. Keep the screening seed 42 separate in sensitivity reporting.
