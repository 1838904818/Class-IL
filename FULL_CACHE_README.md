...
```powershell
python -m fullcache `
  --data-root "C:\path\to\datasets" `
  --output-root "C:\path\to\full_data_cache" `
  --dataset all `
  --chunk-rows 100000 `
  --overlap-work-directory "C:\path\with\enough\temporary\disk"
```
...
The pinned local Malaya checkout uses a separate source root and can be built
without copying either CSVs or PCAPs:

```powershell
python -m fullcache `
  --data-root "C:\path\to\malaya_network_gt_snapshot" `
  --output-root "C:\path\to\formal_v3_cache" `
  --dataset malaya-network-gt `
  --overlap-work-directory "C:\path\to\disk_work"
```

The source root must contain `repo/csv_output/` and the pinned
`candidate_capture_split.json` contract. Only the CSV files are consumed.
...
python -m fullcache.verify `
  --cache-root "C:\path\to\full_data_cache" `
  --dataset all
```
...
python -m fullcache.overlap `
  --cache-root "C:\path\to\full_data_cache" `
  --dataset all `
  --work-directory "C:\path\with\enough\temporary\disk"
```
...
python -m streaming_full `
  --manifest "C:\path\to\full_data_cache\cic-ids-2018\streaming_manifest.json" `
  --config-json .\run_config.json `
  --seeds 11 22 33 44 55 `
  --output-dir .\results\cic-ids-2018
```
