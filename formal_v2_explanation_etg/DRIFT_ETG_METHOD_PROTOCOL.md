...
This is preferable to explaining `s_c` alone because the final prediction is determined by competition between families. The scorer must use population standard deviation (`ddof=0`) to match formal inference. The old archived experiment used a different standard-deviation convention and is therefore not a bit-exact reproduction of the formal deployed score.
