...
class AgentGuardrailTests(unittest.TestCase):
    ...
    def test_large_finite_features_do_not_overflow_input_statistics(self):
        agent = make_agent()
        X = np.array(
            [
                [2.0e38, 1.0e37],
        ...
        agent.fit_input_stats(X)
        normalized = agent._normalize(X)

        self.assertTrue(torch.isfinite(agent.feature_mean).all())
        self.assertTrue(torch.isfinite(agent.feature_std).all())
