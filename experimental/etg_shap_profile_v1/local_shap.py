"""Local-only, hash-bound single-target SHAP profile. Default is validation only.
...
def candidate(path, expected):
    ...
    c = read_json(path)
    check(c['schema'] == 'etg-local-shap-profile-v1' and
          c['scope'] == 'LOCAL_SHAP_PROFILE_NOT_ETG_STUDY', 'candidate scope')
    ...
    check(c['limits'] == {'max_native_calls': 5200, 'native_seconds': 900,
        'process_seconds': 960, 'rss_bytes': 8 * 1024**3, 'gpu_allocator_bytes': 3 * 1024**3,
    ...
    check(c['device'] == 'cuda:0' and c['checkpoints'] == [0], 'fixed diagnostic scope')
    ...
    required += [('repository', 'experimental/etg_shap_profile_v1/' + n) for n in ('local_shap.py', 'shap_core.py')]
...
def approval(path, expected, candidate_sha, c):
    ...
    a = read_json(path)
    check(a.get('verdict') == 'APPROVED' and a.get('scope') == 'local-shap-profile-only'
...
def configure_paths(c):
    ...
    sys.path[:0] = [str(repo / 'experimental/etg_shap_profile_v1'),
                    str(repo / 'experimental/etg_native_profile_v1'),
...
def worker(c, candidate_sha):
    ...
    check(claim['candidate_sha256'] == candidate_sha and claim['parent_pid'] == os.getppid(),
          'worker must be supervised by claiming parent')
    fresh_json(out / 'WORKER_SPENT.json', {'pid': os.getpid(), 'candidate_sha256': candidate_sha})
    configure_paths(c)
    import numpy as np
    import torch
    from bundle import identities, ProfileInputs
    from native_loader import load_native
    from native_target import CallBudget, FixedContextTarget, validate_native
    ...
    selected = inputs.targets[0]
    check(selected['class_id'] == 0 and selected['parent_rows'] == 512, 'fixed first Benign context')
    cp_root = Path(c['roots']['native']) / 'seed_1/checkpoint_000'
    model = load_native(c['roots']['historical_runtime'], cp_root, 0, 'cuda')
    check(model.metadata['seen_classes'] == [0, 1], 'checkpoint axis')
    journal = out / 'progress.jsonl'
    with journal.open('x', encoding='utf-8') as f:
        def record(value):
            journal_record(f, value)
        budget = CallBudget(c['limits']['max_native_calls'], c['limits']['native_seconds'], torch.cuda.synchronize)
        def score(raw):
            result = model.score(raw)
            if budget.calls % 100 == 0:
                record({'stage': 'NATIVE_PROGRESS', 'native_calls': budget.calls,
                        'elapsed_seconds': time.monotonic()-budget.start})
            return result
        target = FixedContextTarget(score, inputs.contexts[0], selected['row_in_parent'], [0, 1], 0, budget)
        result = extract_one(target, inputs.references, c['profile_seed'], record)
        record({'stage': 'SHAP_COMPLETE', 'native_calls': budget.calls})
    inputs.recheck_files()
    result.update(status='SHAP_PROFILE_COMPLETE', schema='etg-single-target-shap-profile-v1',
        candidate_sha256=candidate_sha, checkpoint=0, class_id=0, row_id=selected['row_id'],
        parent_rows=512, reference_rows=32, native_context_rows=budget.context_rows,
        peak_cuda_allocated_bytes=torch.cuda.max_memory_allocated(0),
        peak_cuda_reserved_bytes=torch.cuda.max_memory_reserved(0),
        current_environment={'packages': c['packages'], 'gpu': torch.cuda.get_device_name(0)},
        archived_score_parity_verified=False, full_population_parity_verified=False,
        historical_environment_match=False, shap_attributions_computed=True,
        etg_actions_fitted=False, efficacy_metrics_computed=False, scientific_result_generated=False,
        interpretation='One current-environment attribution resource profile; no drift/noise/ETG efficacy estimate.')
...
def execute(c, candidate_path, candidate_sha, receipt_sha):
    ...
    try:
        ...
        if reason is None:
            ...
            check(result['status'] == 'SHAP_PROFILE_COMPLETE' and 0 < result['native_calls'] <= 5200
                  and len(result['attributions']) == 78 and result['repeat_count'] == 1
                  and result['shap_attributions_computed'] is True and result['candidate_sha256'] == candidate_sha,
                  'incomplete worker result')
            check(all(result[k] is False for k in ('archived_score_parity_verified',
                'full_population_parity_verified',
    ...
    terminal = {'status': 'SHAP_PROFILE_COMPLETE' if reason is None else 'FAILED',
