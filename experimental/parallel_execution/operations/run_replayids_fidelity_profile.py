...
def read_allocated_gpu(gpu_cmd,compute_env):
    """Keep allocation/library context isolated from the minimized SDK context."""
    raw=subprocess.run(gpu_cmd,capture_output=True,text=True,check=True,timeout=5,env=dict(compute_env))
    return parse_gpu(raw.stdout)

def bounded_error(exc,stage):
    """Store codes and hashes only, never raw driver output, commands or environment."""
    record={'stage':stage,'error_type':type(exc).__name__}
    code=getattr(exc,'returncode',None)
    if type(code) is int:record['return_code']=code
    for name in ('stdout','stderr'):
        value=getattr(exc,name,None)
        if isinstance(value,str):value=value.encode('utf-8',errors='replace')
        if isinstance(value,bytes):
            record[name+'_bytes']=len(value)
            record[name+'_prefix_sha256']=hashlib.sha256(value[:8192]).hexdigest()
            record[name+'_hash_truncated']=len(value)>8192
            text=value[:8192].decode('utf-8',errors='replace').lower()
            tags={
                'nvml_initialization_failure':'failed to initialize nvml',
                'driver_library_mismatch':'driver/library version mismatch',
                'no_device':'no devices were found',
                'invalid_device':'invalid device',
                'permission_denied':'permission denied',
            }
            record[name+'_known_error_tags']=[k for k,v in tags.items() if v in text]
    return record
...
def verify_operation(plan,folder):
    if plan.get('schema')!='replayids-fidelity-profile-v3' or plan.get('walltime_seconds')!=600:
        raise ValueError('Unexpected operation contract')
    if plan.get('verifier_deadline_seconds')!=480 or plan.get('allocated_memory_mib')!=8192:
    ...
    if plan.get('wandb')!={'entity':'csnet','project':'ofra-etg-leon-hpc','group':'replayids-score-fidelity-profile-v3'}:
...
def main():
    ...
    status={'schema':'fidelity-profile-operation-v3','job_id':os.environ['SLURM_JOB_ID'],
    ...
    start=time.monotonic();guard=UtilizationGuard(600/3);stage='tracking_environment_precheck'
    ...
    try:
        ...
        with (run_dir/'verifier.stdout.log').open('wb') as out,(run_dir/'verifier.stderr.log').open('wb') as err:
            ...
            while child.poll() is None:
                ...
                try:
                    stage='allocated_gpu_query'
                    sample=read_allocated_gpu(gpu_cmd,compute_env)
                    stage='own_process_tree_measurement'
                    cpu,rss=process_sample(root)
                    sample.update(elapsed_seconds=elapsed,process_tree_rss_bytes=rss,
                        ...
                        host_memory_percent=rss/(plan['allocated_memory_mib']*1024**2)*100)
        ...
        status['error_diagnostic']=bounded_error(exc,stage)
        if stage=='allocated_gpu_preflight':
            status['status']='TELEMETRY_PREFLIGHT_FAILURE';code=EXIT_TELEMETRY
        elif run is None:tracking={'status':'INIT_FAILED','url':None};code=EXIT_TRACKING
    finally:
