...
Data preparation completed on 8 September: all four increments of the prospective P arm and retrospective O comparison passed paired-array validation (prospective_sampling/real_pair_20260908/PAIR_AUDIT.json). P uses a Task-0-only normal cap of 5,559; O retains the historical offline cap of 124,780. Fitting totals are 149,476 and 268,697 rows, respectively, with identical selected attack identities and calibration identities at every increment. The shared calibration set has 68,313 rows; all 227,723 official-test rows remain unchanged. The entire fitting-count difference is 119,221 Task-0 Benign rows. This establishes a prepared sampling contrast, not a trained no-look-ahead result or capture independence.

The common Task-0 normalization and seed-1 FT256x4 encoder have now been fitted only on the 11,118 shared Task-0 fitting rows, with 232 optimizer steps and 88,944 row presentations. The local training artifact audit passed. On 9 September both frozen-representation exports completed and passed the pair audit: P forwarded 377,199 rows in 55.999 seconds and O forwarded 496,420 rows in 70.844 seconds, including the same 227,723 test rows in each. Test-array hashes match across exports and the encoder state remained unchanged (prospective_sampling/real_pair_20260908/EMBEDDING_EXPORT_RESULT.md). These are inference costs, not downstream training or accuracy results.

Downstream P/O models and evaluation remain unexecuted. A read-only schedule calculation exposes an important confound: at ten epochs the existing L1 configuration plans 19,720 optimizer steps and 1,836,910 row presentations per loss arm for P, versus 35,400 and 3,195,890 for O (prospective_sampling/real_pair_20260908/EXPOSURE_PREFLIGHT.md). These are planned, not executed counts. Equal epochs therefore do not equalize exposure; distinguish the practical fixed-epoch comparison from a pre-specified exposure-matched comparison. The L1 head/loss harness has no router and cannot establish full OFRA architectural benefit. Remaining work includes downstream training, matched-budget controls, and verification of the numerical target for subsequent attribution and perturbed inputs; the successful whole-probe ReplayIDS run does not certify the Malaya class-batch attribution path. Complete capture/group provenance and archive missing environment evidence where recoverable. No full-population fidelity, calibration benefit or attribution faithfulness is claimed.
