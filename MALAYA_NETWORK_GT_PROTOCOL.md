...
The first online records are labelled `validation`, not `reproduction`. Only
aggregate metrics, plots, protocol hashes, source revision, and configuration
metadata may be sent to W&B. Raw rows, IP addresses, source CSVs, PCAPs, and
source code remain local.
